...
// MARK: - ResponseVideoEntity
struct ResponseVideoEntity: Codable {
...
extension ResponseVideoEntity {
    static var mock: ResponseVideoEntity {
        let jsonData = ResponseVideoEntity.jsonData.data(using: .utf8)!
        let jsonDecoder = JSONDecoder()
        do {
            let response = try jsonDecoder.decode(ResponseVideoEntity.self, from: jsonData)
            return response
        } catch {
